...
// LineJSON is a framing that transmits and receives records on r and wc, in which
// each record is defined by being a complete JSON value. each valid json call is separated by a newline
func LineJSON(r io.Reader, wc io.WriteCloser) channel.Channel {
...
func (c jsonc) Send(msg []byte) error {
	...
	_, err := c.wc.Write(append(msg, byte('\n')))
	return err
}
